...
| `file-edits.feature`: An agent answering a read-only question cannot change a note                                                                    | When a multi-agent turn asks opencode read-only, Copilot refuses its `edit` and its `write` without showing a card, every file stays byte-identical, and the answer reads "This agent did not answer." The shell is not covered; see Known gaps.                                                                                                                  | [private #12](https://github.com/Brevilabs/obsidian-copilot-private/issues/12), [private #494](https://github.com/Brevilabs/obsidian-copilot-private/issues/494)                                                                      |
| `file-edits.feature`: An agent answering a read-only question can still run a shell command that reads a note                                         | Copilot lets a read-only answer run a shell command, whose output reaches the model, and the answer shows the model's reply. No file changes.                                                                                                                                                                                                                     | [private #138](https://github.com/Brevilabs/obsidian-copilot-private/issues/138)                                                                                                                                                      |
...
- **A read-only answer can change a note through the shell.** A multi-agent
  ...
  `note.md` was overwritten. The user asked a question and lost a note's
  content. The read-only scenarios cover `edit` and `write`, and a shell
...
and the answer is read as `FanoutTurnView` draws a settled answer.
...
`FanoutOrchestrator`, `ToolPermissionCard`, the backend registry and
...
| `obsidianShim.ts` → `FileSystemAdapter` | Obsidian's vault adapter                | Must be a class (`instanceof` checks) rooted at the temp vault. `exists` is called when the session ensures the vault's `AGENTS.md`; `write` when `VaultClient` answers opencode's `fs/write_text_file`.                |
...
outside the agent home in any file-edit scenario.
