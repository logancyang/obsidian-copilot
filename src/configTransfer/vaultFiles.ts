 ...
 * directories). The `copilot-config-*.md` file is AES-256-GCM encrypted and import is
...
export async function restoreVaultFiles(
  ...
  //
  // Design decision: local paths take precedence over imported paths.
  // Using imported paths would silently change the vault's directory
  // structure, orphaning files already in the local folders. The imported
  // settings' folder fields are aligned below so that the subsequent
  // onPersistSettings() call writes consistent paths to data.json.
  ...
  const hasMemory = files.memory.recentConversations != null || files.memory.savedMemories != null;
