...
describe("crypto.ts", () => {
  ...
  describe("decryptWithPassphrase format validation", () => {
    ...
    it("should throw reason='corrupted' for payload exceeding 50MB", async () => {
      // Reason: build a payload just over the size limit.
      // Header (33) + ciphertext must exceed 50MB after base64 decode.
      const oversizedCiphertextLen = 50 * 1024 * 1024;
  ...
  describe("assertConfigPassphrase", () => {
    ...
    it("should not throw for passphrases at or above minimum length", () => {
      expect(() => assertConfigPassphrase("a".repeat(MIN_CONFIG_PASSPHRASE_LENGTH))).not.toThrow();
      expect(() =>
