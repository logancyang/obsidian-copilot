 ...
 *
 * The output is encoded as standard base64 for storage in `copilot-config-*.md` files.
 */
...
/** Maximum accepted encrypted payload size (50 MB). Generous limit since
 *  config files are stored on disk, not transmitted via URI. Kept as a
 *  DoS safeguard against maliciously crafted files. */
const MAX_PAYLOAD_BYTES = 50 * 1024 * 1024;

/** Maximum allowed decompressed plaintext size (200 MB) to prevent zip-bomb DoS. */
 ...
 * is already limited to 50 MB (`MAX_PAYLOAD_BYTES`), which bounds the
 ...
 *
 * @returns A base64-encoded string for storage in `copilot-config-*.md` files.
 */
export async function encryptWithPassphrase(
  ...
  if (estimatedPayload > MAX_PAYLOAD_BYTES) {
    throw new Error(
      "Configuration is too large to export (max 50 MB). " +
...
export async function decryptWithPassphrase(encoded: string, passphrase: string): Promise<string> {
  ...
  try {
    ...
    // This is intentionally kept even though the UI layer (15 MB file limit)
    // and post-decode checks (raw.length, decompressed size) also guard
    // against oversized input — this is the only check BEFORE the base64
    // decode allocation, protecting this helper when called outside the UI.
