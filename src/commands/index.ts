...
export function registerCommands(
  ...
  addCommand(plugin, COMMAND_IDS.INSPECT_COPILOT_INDEX_BY_NOTE_PATHS, async () => {
    try {
      ...
      const VectorStoreManager = (await import("@/search/vectorStoreManager")).default;
      const dbOps = await VectorStoreManager.getInstance().getDbOps();
      const docs = await dbOps.getDocsJsonByPaths([activeFile.path]);

      if (!docs[activeFile.path] || docs[activeFile.path].length === 0) {
        new Notice(`No embedding data found for: ${activeFile.path}`);
        return;
      }

      const chunks = docs[activeFile.path];
      const content = [
