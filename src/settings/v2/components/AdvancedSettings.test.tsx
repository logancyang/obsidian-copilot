  ...
  useModelManagement: () => ({
    providerRegistry: {
      notifyCredentialStoreChanged: (providerIds: readonly string[]): void => {
        notifyCredentialStoreChanged(providerIds);
      },
    },
  }),
}));
...
describe("AdvancedSettings", () => {
  describe("AdvancedSettings()", () => {
    ...
    it("counts a BYOK provider's keychain entry so a BYOK-only setup is not reported as keyless (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", () => {
    ...
    // key. https://github.com/Brevilabs/obsidian-copilot-private/issues/210
    it("announces the credential wipe through the registry even when the transaction fails after the sweep ran (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", async () => {
      forgetAllSecrets.mockImplementation(async (_saveData, syncMemory) => {
        ...
        syncMemory({ ...DEFAULT_SETTINGS, providers: { p1: byokProvider } });
      ...
      // Subscribers resolve the row by id, so the announcement has to name the
      // providers whose credential the sweep removed.
      await waitFor(() => expect(notifyCredentialStoreChanged).toHaveBeenCalledTimes(1));
      expect(notifyCredentialStoreChanged).toHaveBeenCalledWith(["p1"]);
    });

    it("names the providers the sweep actually saw, not the ones this render knew about (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", async () => {
      // Confirmation and the persistence queue both await, so a provider added
      // in between is cleared by the sweep while the render snapshot that
      // started the handler still has no record of it.
      forgetAllSecrets.mockImplementation(async (_saveData, syncMemory) => {
        syncMemory({ ...DEFAULT_SETTINGS, providers: { p1: byokProvider } });
      });
      seedSettings({});
      ...
      await waitFor(() => expect(notifyCredentialStoreChanged).toHaveBeenCalledWith(["p1"]));
    });

    it("does not announce a credential wipe when the transaction returns without touching the stores (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", async () => {
