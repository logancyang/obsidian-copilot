 ...
 * link can never resolve or be mistaken for a real Brevilabs response. Swap
 * this module out for a real adapter once the endpoint's contract (auth,
 * retention, idempotent retry) is settled — see
 * `designdocs/REPORT_UPLOAD_FLOW.md`.
 */
export const mockReportUploader: ReportUploader = async (): Promise<ReportUploadResult> => {
  ...
  return {
    shareUrl: `https://copilot-reports.invalid/r/${Math.random().toString(36).slice(2, 10)}`,
