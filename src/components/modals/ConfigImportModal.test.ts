...
jest.mock("@/configTransfer/vaultFiles", () => ({
  restoreVaultFiles: jest.fn().mockResolvedValue({
    commandsWritten: 0,
    promptsWritten: 0,
    memoryWritten: 0,
    errors: [],
...
describe("ConfigImportModal", () => {
  ...
  it("preserves the current vault userId during import", async () => {
    // Reason: userId identifies the local vault; importing a foreign userId
    // would reset the vault's identity and break analytics / per-vault state.
    const onPersist = openAndCapture();
    ...
    expect(mergedSettings.userId).toBe("local-user-id");
  });
