...
function ContextManage({ initialProject, onSave, onCancel, app }: ContextManageProps) {
  ...
  const [ignoreItems, setIgnoreItems] = useState<DeletedItems>(() => {
    // init exclude files
  ...
  // file items convert to exclusions format
  const convertDeletedItemsToExclusions = useCallback((items: DeletedItems): string => {
  ...
  const addPatternToGroup = useCallback(
    (
      ...
      // todo(em-lin): remove files or remove file from ignore
      /*const matchingFiles = getMatchingFilesFromApp(patternConfig)
      const needRemoveFilesFromIgnore =
        [...latestIgnoreItems.current.files].filter((it) =>
          matchingFiles.some((f) => f.id === it.path)
        );

      setIgnoreItems((prev) => {
        prev.files = new Set([...prev.files].filter((it) => !needRemoveFilesFromIgnore.includes(it)));
        return prev;
      });*/
