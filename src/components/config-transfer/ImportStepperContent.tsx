...
/** Maximum file size for config files (75 MB). Generous limit since config
 *  files are stored on disk. Prevents OOM before crypto checks. */
const MAX_CONFIG_FILE_SIZE = 75 * 1024 * 1024;
export const ImportStepperContent: React.FC<ImportStepperContentProps> = ({
    ...
    async (file: VaultConfigFile) => {
      ...
      try {
        if (file.size > MAX_CONFIG_FILE_SIZE) {
          setErrorMessage("File is too large (max 75 MB).");
          return;
        }
  ...
  useEffect(() => {
    const files = appInstance.vault
      .getFiles()
      .filter((f) => f.extension === "md" && f.name.startsWith("copilot-config-"))
      .map((f) => ({
        name: f.name,
        path: f.path,
        size: f.stat.size,
  ...
  const handleNextFromPassword = async () => {
    ...
    try {
      const {
        ...
        meta,
        vaultFiles: files,
      } = await decryptConfigFile(parsedWrapper, password);
  ...
  const handleImport = async () => {
    ...
    try {
      const importedSettings =
        cachedSettingsRef.current ?? (await decryptConfigFile(parsedWrapper!, password)).settings;
        ...
        <div className="tw-flex tw-flex-col tw-gap-5">
          <div className="tw-flex tw-items-start tw-gap-3 tw-rounded-md tw-border tw-border-[var(--background-modifier-error-hover)] tw-p-4 tw-bg-modifier-error/10">
            ...
            <div className="tw-flex tw-flex-col">
              ...
              <span className="tw-mt-1 tw-text-xs tw-text-normal">
                ...
                same-named custom commands, system prompts, and memory files. Existing files not
                included in the import will be kept.
              </span>
            </div>
          </div>
          ...
          <label className="tw-group tw-flex tw-cursor-pointer tw-items-start tw-gap-3">
            ...
            <span className="tw-text-sm tw-leading-relaxed tw-text-muted tw-transition-colors group-hover:tw-text-normal">
              I understand that my current settings will be replaced and same-named files
              overwritten
            </span>
          </label>
