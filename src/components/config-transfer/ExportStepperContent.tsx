...
import { type ExportContentOptions, DEFAULT_EXPORT_OPTIONS } from "@/configTransfer/vaultFiles";
...
export const ExportStepperContent: React.FC<ExportStepperContentProps> = ({
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      const content = await generateConfigFile(appInstance, password, pluginVersion, exportOptions);
  ...
  return (
    <div ref={containerRef} className="tw-flex tw-flex-col tw-gap-6" style={{ minHeight }}>
      ...
      <div className="tw-text-sm tw-leading-relaxed tw-text-muted">
        Encrypt your settings, API keys, and selected Copilot files as a portable file.
      </div>
