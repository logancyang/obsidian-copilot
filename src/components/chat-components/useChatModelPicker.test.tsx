...
describe("useChatModelPicker", () => {
  describe("useChatModelPicker()", () => {
    ...
    it("gates the model behind 'Add API key' once Delete All Keys removes the entry, then clears the gate when the key is re-entered under the same pointer (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", async () => {
      secrets.set(KEYCHAIN_ID, "sk-live");
      const { result } = renderPicker();
      expect(disabledReason(result)).toBeUndefined();
    ...
    it("keeps a keyless provider's model selectable when it owns no credential pointer", () => {
      settingsStore.set(settingsAtom, (prev) => ({
        ...prev,
        providers: {
          p1: { ...prev.providers.p1, requiresApiKey: false, apiKeyKeychainId: null },
        },
      }));
      const { result } = renderPicker();
      expect(disabledReason(result)).toBeUndefined();
    ...
    it("gates an optional-auth provider that still owns a keychain pointer (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", () => {
      // Owning a pointer means the runtime resolves a credential for this row
      // and refuses the turn when nothing comes back, so declaring auth
      // optional must not present the model as ready to use.
      settingsStore.set(settingsAtom, (prev) => ({
        ...prev,
        providers: {
