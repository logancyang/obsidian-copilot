...
export function useChatModelPicker(params: {
  ...
  const { models, byModelKey, idToModelKey } = React.useMemo(() => {
    ...
    for (const entry of entries) {
      ...
      // Reason: the runtime refuses any provider that must resolve a credential,
      // which includes an optional-auth provider that still owns a keychain
      // pointer — `providerRequiresApiKey` alone would call that row usable.
      const needsKey =
        providerNeedsResolvedApiKey(provider) && !providerHasApiKey(provider, keychain);
