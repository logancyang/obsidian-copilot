...
describe("providerHasApiKey", () => {
  describe("providerHasApiKey()", () => {
    ...
    it("returns false for a missing entry — the pointer alone is not evidence (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", () => {
    ...
    it("returns false for a tombstoned ('') entry (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", () => {
      expect(providerHasApiKey(provider(), keychainWith(""))).toBe(false);
    });

    it("returns false for a whitespace-only entry, matching what verification calls missing (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", () => {
      expect(providerHasApiKey(provider(), keychainWith("   "))).toBe(false);
    });

    it("returns false when the keychain read throws — rendering must not (https://github.com/Brevilabs/obsidian-copilot-private/issues/210)", () => {
      const keychain = {
