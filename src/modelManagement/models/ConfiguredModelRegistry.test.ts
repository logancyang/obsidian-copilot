...
function seedByokProviders(): void {
  setSettings({
    providers: {
      byok: {
        providerId: "byok",
...
describe("models/ConfiguredModelRegistry", () => {
  describe("ConfiguredModelRegistry", () => {
    ...
    describe("add()", () => {
      it("mints id, stamps configuredAt, appends row", async () => {
      ...
      it("enforces (providerId, info.id) uniqueness", async () => {
      ...
      it("rejects embeddings only for BYOK (https://github.com/Brevilabs/obsidian-copilot-private/issues/386)", async () => {
        seedByokProviders();
    ...
    describe("update()", () => {
      it("merges info patch and rejects unknown id", async () => {
        const id = await registry.add({
          providerId: PROVIDER_A,
      ...
      it("cannot turn a saved BYOK model into an embedding (https://github.com/Brevilabs/obsidian-copilot-private/issues/386)", async () => {
        seedByokProviders();
    ...
    describe("remove()", () => {
      it("drops the row; idempotent for unknown id", async () => {
      ...
      it("settings reflect mutations atomically", async () => {
        const id = await registry.add({ providerId: PROVIDER_A, info: info("m1") });
        expect(
          getSettings().configuredModels.find((m) => m.configuredModelId === id)
        ).toBeDefined();
        await registry.remove(id);
        expect(
          getSettings().configuredModels.find((m) => m.configuredModelId === id)
    ...
    describe("removeByProvider()", () => {
      it("only touches rows under the target provider", async () => {
    ...
    describe("listByProvider()", () => {
      it("returns stable references when settings unchanged", async () => {
        await registry.add({ providerId: PROVIDER_A, info: info("m1") });
    ...
    describe("list()", () => {
      it("reuses a stable empty reference when no rows are configured", () => {
    ...
    describe("getByWireId()", () => {
      it("resolves under the right provider only", async () => {
        const a1 = await registry.add({ providerId: PROVIDER_A, info: info("m1") });
    ...
    describe("bulkSet()", () => {
      it("preserves configuredModelId for existing (providerId, info.id) matches", async () => {
      ...
      it("refreshes info for reused rows while keeping configuredModelId stable", async () => {
      ...
      it("reuses the row reference when the fresh info is structurally equal", async () => {
      ...
      it("silently dedupes duplicate info.id entries in the input", async () => {
      ...
      it("does not touch rows belonging to other providers", async () => {
        const a1 = await registry.add({ providerId: PROVIDER_A, info: info("m1") });
        const b1 = await registry.add({ providerId: PROVIDER_B, info: info("m1") });
      ...
      it("rejects a mixed replacement without losing existing models (https://github.com/Brevilabs/obsidian-copilot-private/issues/386)", async () => {
        seedByokProviders();
