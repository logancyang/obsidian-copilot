  ...
  ({
    ...
    return (
      <div className="tw-relative tw-flex tw-h-full tw-flex-1 tw-flex-col tw-overflow-hidden">
        {/* The jump-back button anchors to the message region, not to the pane:
            the action rail below is a sibling, and a button positioned against
            the pane would float over a pending permission card and cover the
            controls that rail exists to keep reachable. */}
          ...
          >
            <div ref={contentCallbackRef} className="tw-w-full">
              {visible.map((message, index) => {
                ...
                const completedTurnDurationMs = ownsTurnDuration
