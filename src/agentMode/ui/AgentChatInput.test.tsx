...
describe("AgentChatInput", () => {
  ...
  describe("handleSendMessage()", () => {
    ...
    it("keeps the draft and its attachments in the composer when the agent is too old to run (https://github.com/Brevilabs/obsidian-copilot-private/issues/480)", async () => {
      jest.mocked(Notice).mockClear();
      ...
      renderInput(backend, draft, {
        unsupportedAgentMessage: "opencode 1.0.0 is outdated. Update to 1.18.31.",
      });
      await act(async () => fireEvent.click(screen.getByText("send")));
      ...
      expect(draft.resetCompose).not.toHaveBeenCalled();
      expect(draft.setQueue).not.toHaveBeenCalled();
      expect(Notice).toHaveBeenCalledWith("opencode 1.0.0 is outdated. Update to 1.18.31.");
    });
  ...
  describe("AgentChatInput()", () => {
    it("holds queued follow-ups instead of flushing them when the agent is too old to run (https://github.com/Brevilabs/obsidian-copilot-private/issues/480)", async () => {
      const backend = { sendMessage: jest.fn(), cancel: jest.fn() } as unknown as AgentChatBackend;
      const draft = makeDraft({
        input: "",
