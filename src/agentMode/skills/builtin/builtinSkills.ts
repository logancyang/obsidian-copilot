...
const SYMPOSIUM_PUBLISH: BuiltinSkill = {
  ...
  skillMd: `---
name: symposium-publish
description: Convert an existing Markdown note into standalone HTML, then hand the finished page to Obsidian for exact-byte review and trusted Symposium publishing or updating. Use when the user asks to publish or share Markdown as a web page.
metadata:
  copilot-enabled-agents: claude, codex, opencode
...
Obsidian rejects active or externally loaded content, prevents navigation from the
browser preview, removes the original artifact, removes its temporary browser preview
after the review, and alone reads the current note identity to choose whether
confirmation publishes or updates; never choose an action or document id.
...
- \`failed\`: stop after this attempt and report the exact host message. Do not guess,
  rewrite valid CSS, retry, bypass the review, or publish directly unless the user asks.
`,
  files: [
    {
...
if ($args.Count -ne 2 -or -not $SOURCE -or -not $STAGED_HTML) {
