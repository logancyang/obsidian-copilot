...
describe("AgentSessionManager.applyMode", () => {
  function buildModeSpec() {
    return { kind: "setMode", nativeId: "plan" } as const;
  }

  it("queues a mode switch fired while the session is still starting", async () => {
    const mgr = buildManager();
    ...
    sessionCreateSpy.mockImplementationOnce((opts) =>
      makeMockSession({ internalId: opts.internalId, backendId: opts.backendId, ready })
    );
    const session = await mgr.createSession();
    const setMode = session.setMode as jest.Mock;

    const pending = mgr.applyMode("opencode", "plan", buildModeSpec());
    // Drain microtasks so the awaiting code reaches `await session.ready`.
    await Promise.resolve();
    await Promise.resolve();
  ...
  it("skips dispatch when the active session changes while waiting on ready", async () => {
    const mgr = buildManager();
    let resolveReady: (() => void) | null = null;
    const ready = new Promise<void>((resolve) => {
      resolveReady = resolve;
    ...
    const a = await mgr.createSession();
    // A second, already-ready session to swap to before `a.ready` resolves.
    sessionCreateSpy.mockImplementationOnce((opts) =>
      makeMockSession({ internalId: opts.internalId, backendId: opts.backendId })
    );
    const b = await mgr.createSession();
...
describe("AgentSessionManager.applySelection (deferred ready)", () => {
  it("queues a selection apply fired while the session is still starting", async () => {
    const applySelectionMock = jest.fn(async () => {});
    const descriptor = {
      id: "opencode",
      displayName: "opencode",
    ...
    const pending = mgr.applySelection({ effort: "high" });
    await Promise.resolve();
    await Promise.resolve();
