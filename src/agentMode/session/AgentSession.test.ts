...
describe("AgentSession.sendPrompt", () => {
  it("rejects a send without changing history when readiness becomes incompatible (https://github.com/Brevilabs/obsidian-copilot-private/issues/531)", async () => {
    const mock = makeMockBackend();
    const descriptor = {
...
describe("AgentSession.create (via start)", () => {
  it("does not open a backend session when readiness becomes incompatible during context loading (https://github.com/Brevilabs/obsidian-copilot-private/issues/531)", async () => {
    const mock = makeMockBackend();
    const descriptor = {
      getInstallState: jest.fn(() => ({ kind: "ready" })),
    } as unknown as BackendDescriptor;
