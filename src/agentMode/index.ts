...
export function createAgentSessionManager(app: App, plugin: CopilotPlugin): AgentSessionManager {
  ...
  for (const descriptor of listBackendDescriptors()) {
    descriptor.subscribeInstallState(plugin, () => {
      // Install preparation and the runtime refresh share the manager's ordered pipeline, so a
      // second install cannot overtake the first. https://github.com/logancyang/obsidian-copilot/issues/3022
