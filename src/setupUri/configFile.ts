/**
 * Core logic for generating and parsing .copilot configuration files.
 *
...
/** Outer JSON wrapper of a .copilot file. */
...
/**
 * Generate a .copilot configuration file containing settings + vault files.
 *
 ...
 * @returns JSON string of the .copilot file wrapper.
 */
...
/**
 * Parse the outer wrapper of a .copilot file (no decryption needed).
 *
 ...
 * @throws {Error} If the content is not a valid .copilot file.
 */
export function parseConfigFileWrapper(content: string): ConfigFileWrapper {
  ...
  if (obj.format !== CONFIG_FILE_FORMAT) {
    throw new Error(
      "The selected file is not a valid Copilot configuration file " +
        "(unrecognized format)."
    );
  }
...
/**
 * Decrypt and parse the inner payload of a .copilot file.
 *
...
function parsePortableFiles(
  ...
  return input
    ...
    .filter(
      (item) => typeof item.filename === "string" && typeof item.content === "string"
    )
    .map((item) => ({
...
function parsePortableMemory(input: unknown): {
  ...
  return {
    ...
    savedMemories:
      typeof mem.savedMemories === "string" ? mem.savedMemories : null,
  };
}
