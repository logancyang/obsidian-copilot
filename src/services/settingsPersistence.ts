...
/**
 * Monotonic counter incremented by each `runPersistenceTransaction()`.
 *
 * Reason: a persist job queued before a transaction may carry a stale snapshot
 * (e.g. with secrets that the transaction just deleted). When the job finally
 * executes inside `doPersist`, it compares the epoch it captured at enqueue time
 * against the current value. A mismatch means a transaction committed in between,
 * so the stale job is dropped.
 */
...
export async function runPersistenceTransaction(task: () => Promise<void>): Promise<void> {
  const job = writeQueue.then(async () => {
    try {
      ...
      // Reason: bump even on failure so that any persist job queued during the
      // transaction is dropped. A failed transaction may have partially mutated
      // disk state (e.g. forgetAllSecrets stripped data.json before keychain
      // clear threw); letting a stale persist run would overwrite that state.
      transactionEpoch++;
    }
  });
...
export async function persistSettings(
  ...
  const job = writeQueue.then(() => {
    // Reason: if a transaction committed after this job was queued, the captured
    // settings snapshot is stale and may contain secrets the transaction deleted.
