...
// Pending file writes: prevent event loops when writing frontmatter (aligned with system-prompts)
...
export function addPendingFileWrite(path: string): void {
  pendingFileWrites.add(normalizePath(path));
}
...
export function removePendingFileWrite(path: string): void {
  pendingFileWrites.delete(normalizePath(path));
}
...
export function isPendingFileWrite(path: string): boolean {
  return pendingFileWrites.has(normalizePath(path));
}
