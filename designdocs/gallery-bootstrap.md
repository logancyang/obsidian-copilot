...
|    6 | IN PROGRESS | `codex/gallery-bootstrap-step-6`                | PASS automated: three permanent story modules across two presentational roots (`Button`, `Badge`, `AgentWelcomeCard`); 5 focused gallery suites / 28 tests; full unit suite (371 suites / 5,256 tests); format; lint; clean TypeScript; gallery and production builds; exact Step 5 production `main.js` and `styles.css` SHA-256 matches from a clean detached baseline; gallery bundle has no LangChain marker; production artifacts have no gallery chrome/story markers; generated index contains exactly the three intended modules; diff check; imported spec body byte-identical. Focused behavior covers visible nested navigation and selected styling, exact current story id, title/story filtering, mouse and arrow selection, one story at a time, close-tab/command-reopen persistence, leaf-only subtree contact sheets, 300/340/400/600 widths, theme note, and padded/centered/fullscreen layouts; `Button/Sizes` explicitly wraps at 300px. Adaptations: compose tracked gallery-only width CSS into the ignored development stylesheet without changing production CSS; preserve workspace ItemView state and also pass the last ItemView state to brand-new command-created leaves; pull `dev/gallery` into `tsconfig.json` now because this is the first step that introduces gallery TSX, rather than leaving those files unparsed until the later hygiene milestone. A live Things-light attempt exposed native theme CSS overriding the raw navigation buttons' selected colors, so both subtree and story navigation now reuse the production `Button` primitive (`default` selected, `ghost2` unselected); live recheck pending. |
|    7 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|    8 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|    9 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|   10 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
