...
|    7 | IN PROGRESS | `codex/gallery-bootstrap-step-7` / — | PASS automated: four permanent host stories (`leaf`, native `modal`, anchored `popover`, and `settings-tab`) with adjacent realistic fixtures; 6 focused gallery suites / 32 tests; full unit suite (372 suites / 5,260 tests); format; lint; clean TypeScript; gallery and production builds; exact Step 6 production `main.js` and `styles.css` SHA-256 matches from a clean archive baseline (`a757a91d…`, `18e86cba…`); gallery bundle has no LangChain marker; production bundle has no gallery/host-story markers; generated index contains exactly the four intended modules; diff check; imported spec body byte-identical. Modal opening is edge-triggered, closing retains the selected story, and selecting away/back opens a fresh native modal; popovers portal into the trigger's owning document; contact sheets render non-leaf launch cards instead of story content. Live Obsidian was not run because this task explicitly prohibited vault and GUI access. Adaptations: keep `GalleryProviders` implemented only in `dev/gallery` and use one documented story-only import exception; keep host fixtures as a bonus module so presentational coverage remains unchanged; extend the isolated main-test Obsidian mock with the newly imported `Modal` boundary. |
|    8 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|    9 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|   10 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
