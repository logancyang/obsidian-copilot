...
|    5 | COMPLETE | `codex/gallery-bootstrap-step-5` / [PR #2731](https://github.com/logancyang/obsidian-copilot/pull/2731) | PASS automated: a missing generated index was recreated before `gallery:dev` started; temporary story-label and Tailwind-utility edits rebuilt the gallery JS and CSS watchers, then were removed without source residue; gallery build; gallery-directory deploy fixture covering known legacy-directory migration, unexpected-entry refusal without mutation, and existing-symlink repointing; 3 focused suites / 13 tests; full unit suite (369 suites / 5,241 tests); format; lint; production build; shell syntax; bundle boundaries; production artifacts and entry unchanged; diff check; imported spec body byte-identical. Live Obsidian, only in `copilot-test-vault`: attempt 1 proved JS/CSS rebuilt but Hot Reload 0.3.0 did not receive changes from individually symlinked artifacts on macOS; after linking the whole plugin directory, its watcher was active, one story save replaced the loaded gallery plugin instance and displayed `Hot reload verified` plus the newly generated `.tw-border-double` rule with no build command or manual toggle; reverting auto-restored `Working…`, the final full build removed all probe JS/CSS, one gallery leaf and production Copilot Chat remained live. Screenshot: `step-5-hot-reload.png`. Adaptations: shared `gallery:stories` / `gallery:esbuild` scripts; Tailwind polling because native filesystem events were silent in the isolated worktree; guarded migration refuses unknown plugin files before mutation; ignored source `.hotreload` plus directory symlink satisfies the real macOS watcher. |
|    6 | COMPLETE | `codex/gallery-bootstrap-step-6` / [PR #2732](https://github.com/logancyang/obsidian-copilot/pull/2732) | PASS automated: three permanent story modules across two presentational roots (`Button`, `Badge`, `AgentWelcomeCard`); 5 focused gallery suites / 28 tests; full unit suite (371 suites / 5,256 tests); format; lint; clean TypeScript; gallery and production builds; exact Step 5 production `main.js` and `styles.css` SHA-256 matches from a clean detached baseline; gallery bundle has no LangChain marker; production artifacts have no gallery chrome/story markers; generated index contains exactly the three intended modules; diff check; imported spec body byte-identical. Live Obsidian, only in `copilot-test-vault`: the always-visible tree listed all 7 stories across Agent Welcome Card, Badge, and Button; click and ArrowDown selection, title/story filtering, the 5-leaf UI contact sheet, one-story rendering, 300px Button wrapping, and close-tab/command-reopen persistence all passed. Default light, Things light, and the restored original Things/System dark theme preserved selection and layout. A Things-light check exposed native theme CSS overriding raw navigation-button colors; reusing the production `Button` primitive fixed the selected item to the actual accent/white computed colors in both Things light and dark. The test vault appearance file was restored byte-for-byte to its original SHA-256, and production Copilot Chat remained live. Screenshot: `step-6-navigation.png`. Adaptations: compose tracked gallery-only width CSS into the ignored development stylesheet without changing production CSS; preserve workspace ItemView state and also pass the last ItemView state to brand-new command-created leaves; pull `dev/gallery` into `tsconfig.json` now because this is the first step that introduces gallery TSX, rather than leaving those files unparsed until the later hygiene milestone. |
|    7 | COMPLETE | `codex/gallery-bootstrap-step-7` / [PR #2733](https://github.com/logancyang/obsidian-copilot/pull/2733) | PASS automated: four permanent host stories (`leaf`, native `modal`, anchored `popover`, and `settings-tab`) with adjacent realistic fixtures, including real-catalog assertions for a fullscreen provider-backed leaf and a padded modal; 6 focused gallery suites / 32 tests; full unit suite (372 suites / 5,260 tests); format; lint; clean TypeScript; gallery and production builds; exact Step 6 production `main.js` and `styles.css` SHA-256 matches from a clean archive baseline (`a757a91d…`, `18e86cba…`); gallery bundle has no LangChain marker; production bundle has no gallery/host-story markers; generated index contains exactly the four intended modules; diff check; imported spec body byte-identical. Live Obsidian, only in `copilot-test-vault`: the modal story opened a real native modal with its close control and backdrop, Escape closed it while keeping the same story selected, and reopen plus select-away/back worked; the anchored popover, settings-tab ancestry, provider-backed composite, padded layout, and three non-leaf contact-sheet launchers all passed without inline-rendering non-leaf content. The first pass exposed that no permanent story declared `fullscreen`; making `DefaultLeaf` a permanent fullscreen story produced the expected `min-h-full` / `w-full` container on recheck. The final stable `dev:errors` buffer was clean, the test vault appearance remained byte-identical to its original Things/System SHA-256, and one production Copilot Chat remained live. Screenshot: `step-7-modal-host.png`. Adaptations: keep `GalleryProviders` implemented only in `dev/gallery` and use one documented story-only import exception; keep host fixtures as a bonus module so presentational coverage remains unchanged; extend the isolated main-test Obsidian mock with the newly imported `Modal` boundary. |
|    8 | COMPLETE | `codex/gallery-bootstrap-step-8` / [PR #2735](https://github.com/logancyang/obsidian-copilot/pull/2735) | PASS automated: typed async `list()` / `show()` / `audit()` handle; stable selectors and keyed error isolation across all four real hosts; test-only broken and 300px overflow probes reported as data with no permanent probe residue; 3 focused suites / 41 tests; full unit suite (373 suites / 5,274 tests); format; lint; clean TypeScript; gallery and production builds; exact Step 7 production `main.js` and `styles.css` SHA-256 matches (`a757a91d…`, `18e86cba…`); gallery bundle has no LangChain marker; production artifacts have no gallery handle/selectors/probes; generated index contains exactly the four intended modules; diff check; imported spec body byte-identical. Live Obsidian CLI, only in `copilot-test-vault`: `list()` returned all 11 permanent IDs; after closing the gallery leaf, `show("UI/Button/Variants", { width: 512 })` created and revealed one gallery view beside production Copilot Chat and exposed the exact stable selector. The first three audit attempts exposed external-operation overlap, a retained Radix `data-state="closed"` popover node, and throttleable background layout settlement; the rejection-safe queue, lifecycle-signal host close, and `MessageChannel` fallback fixed each root cause. After an explicit reload of only the test-vault window to discard orphaned pre-fix promises, one store-and-poll `audit({ widths: [300] })` fulfilled in about two seconds with a `Things-dark` report across all stories: 18 contrast findings and no overflow, render failure, off-token color, or zero-size findings. It restored the prior host state; a post-audit queued `show()` then fulfilled and selected Button Variants at 300px. `dev:errors` was clean, the test-vault appearance file retained its original SHA-256, and one production Copilot Chat remained live. No screenshot is attached because this milestone's deliverable is explicitly screenshot-free machine-readable verification. Adaptations: load one shared catalog during plugin `onload`; serialize external operations before driving the selected view; accept positive dynamic widths while retaining four visible controls; read community-theme identity without exposing theme mutation; ignore alpha-zero backgrounds. |
|    9 | COMPLETE | `codex/gallery-bootstrap-step-9` / [PR #2736](https://github.com/logancyang/obsidian-copilot/pull/2736) | PASS automated: forbidden Gallery value-import probe failed with the custom boundary message while the same type-only import passed; a deliberate `Gallery.tsx` type error failed `tsc`; 7 focused suites / 48 tests; format; lint; clean TypeScript; gallery and production builds; exact parent production SHA-256 matches (`a757a91d…`, `18e86cba…`); generated artifacts ignored while stories remain trackable; diff check; imported spec body byte-identical. `lint:dead` remains at 22 pre-existing unused files with no Gallery file finding; expanded coverage also surfaces the pre-existing public `GalleryWidth` type from byte-unchanged Step 8 source, which is preserved. Live Obsidian, only in `copilot-test-vault`: the independently deployed Gallery plugin loaded with all 11 permanent story IDs, one Gallery leaf and one production Copilot Chat remained present, Button Variants stayed mounted at 300px, and the story/dev error buffers were clean. The test vault retained Things and its original appearance-file SHA-256. No screenshot is attached because this milestone changes tooling only and has no visual deliverable. Adaptation: exempt erased test fixtures, retain the plan's UI/lib/utils surface, and allow only the existing `ReactModal`, `@/context`, and `mountPluginViewRoot` runtime seams rather than settings, state, or models. |
|   10 | COMPLETE    | `codex/gallery-bootstrap-step-10` / [PR #2737](https://github.com/logancyang/obsidian-copilot/pull/2737) | PASS automated: story-first gate captured with only `AgentStatusCard.stories.tsx` present and `tsc` failing at its not-yet-created component import; six deterministic 300px stories cover checking, install action, incompatible warning, long error/retry, disabled upgrade, and linked sign-in; 2 focused component/integration suites / 11 tests; all 7 `dev/gallery` suites / 48 tests; full Jest 374 suites / 5,286 tests; format; lint; clean TypeScript; Gallery and production builds; generated index auto-discovers `AgentStatusCard` with no registry edit; no arbitrary Tailwind bracket classes or literal CSS colors in the component/story; production contains the integrated component shell but no Gallery chrome/API/story markers. Production `main.js` changed from Step 9 `a757a91d…` to `2d39c6ad…`; tokenized semantic borders and the full-opacity disabled state changed `styles.css` from `18e86cba…` to `40470d1f…`. `lint:dead` retains the 22-file pre-existing baseline with no new component/story/Gallery finding; diff check and imported spec body byte-identity pass. First live default-light audit at 300px found contrast failures only: warning text 3.0:1, default actions 3.4:1, linked sign-in 3.9:1, and error text 3.9:1; overflow, render failure, off-token color, and zero-size findings were all zero. The token-only repair keeps native alert roles and semantic warning/error icons, renders messages with normal text, uses secondary neutral/warning/link actions, and preserves disabled contrast at full opacity. Live recheck is clean for all six new story IDs across default light, default dark, Baseline light, and Things dark, with zero contrast, overflow, render-failure, off-token-color, or zero-size findings. A subsequent Agent Status Card contact-sheet check isolated one Gallery-chrome overflow: `IncompatibleWarning` measured `clientWidth = 300`, `scrollWidth = 313` because the exact story ID was unbreakable while the component itself stayed within bounds. The general Gallery header repair lets both the readable story name and full exact ID shrink and wrap; final live contact-sheet measurements are `clientWidth = scrollWidth = 300` for all six states with no render errors. The verified Step 10 production runtime and separate Gallery each remained loaded in `copilot-test-vault`, one production Copilot Chat and one Gallery leaf were present, and `dev:errors` was clean. The final live production identity was `9f3cfb75-clean-4e8840e85aa9`; the test vault was restored byte-for-byte to its original Things/System appearance SHA-256. Screenshot: `step-10-agent-status-card.png`. |

Step 10 follow-up: replaced five invalid `tw-border-{side} tw-border-solid` Gallery dividers that activated default widths on every side with explicit one-side rules, as required by the repository styling reference. Focused Gallery/CSS tests pass (21 tests); full Jest passes (374 suites / 5,287 tests); format, lint, TypeScript, Gallery build, and production build pass; production artifact hashes remain unchanged. In `copilot-test-vault`, computed styles show exactly `1px solid` on each intended side and `0px none` elsewhere, all six Agent Status Card stories remain 300px without overflow, `dev:errors` is clean, and the PR screenshot is replaced with the corrected border rendering.

Step 10 follow-up: viewport width is now user-owned state instead of story metadata, so every preset and an arbitrary positive integer custom width persist while selecting other stories. The header adds a custom-pixel input with Apply/Enter support and a 1920px hint for a 1080p-wide viewport; it removes the redundant current-width and Obsidian-theme explanations. The sidebar removes the global selected-story/contact-sheet toggle and `Subtree:` label while retaining direct contact-sheet selection on component-tree headings. Focused Gallery/ItemView tests pass (2 suites / 40 tests); full Jest passes (374 suites / 5,288 tests); format, lint, TypeScript, Gallery build, and production build pass; production artifact hashes remain unchanged. In `copilot-test-vault`, 600px survived a switch to a 300px-metadata story and custom 1920px survived a second story switch; Enter kept focus on the input, Apply kept focus on its button, and the removed controls and copy were absent from the live DOM. The final Agent Status Card contact sheet rendered all six stories at custom 720px without overflow, `dev:errors` was clean, production Copilot Chat remained a separate live pane, the vault appearance SHA-256 remained byte-identical, and the PR screenshot was replaced with this verified state.
