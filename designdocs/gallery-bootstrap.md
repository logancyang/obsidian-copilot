...
|    8 | IN PROGRESS | `codex/gallery-bootstrap-step-8`               | PASS automated: typed async `list()` / `show()` / `audit()` handle; stable selectors and keyed error isolation across all four real hosts; test-only broken and 300px overflow probes reported as data with no permanent probe residue; 3 focused suites / 41 tests; full unit suite (373 suites / 5,274 tests); format; lint; clean TypeScript; gallery and production builds; exact Step 7 production `main.js` and `styles.css` SHA-256 matches (`a757a91d…`, `18e86cba…`); gallery bundle has no LangChain marker; production artifacts have no gallery handle/selectors/probes; generated index contains exactly the four intended modules; diff check; imported spec body byte-identical. Adaptations: load one shared catalog during plugin `onload`; serialize externally requested `show()` / `audit()` operations through one rejection-safe plugin queue before they drive the selected `GalleryView`; close non-leaf hosts from their explicit lifecycle signal plus two bounded layout frames rather than DOM disappearance, then let the next keyed story unmount retained host nodes; settle layout in hidden or unfocused windows with a `MessageChannel` task instead of throttleable animation-frame and timer callbacks; restore pre-audit state; accept positive dynamic canvas widths while retaining the four existing controls; read community-theme identity from `this.app.vault.getConfig("cssTheme")` without exposing a theme mutation; ignore alpha-zero computed backgrounds so transparent descendants do not create off-token false positives. Live Obsidian attempts: (1) FAIL — the CLI returned before the first audit visibly settled, a second audit overlapped it, and the view remained at `Gallery/Host Environments/DeleteConfirmation` / 300px with a native modal open and no report assignment after more than 12 seconds; (2) FAIL — a single queued audit remained at `Gallery/Host Environments/ResponseActions` after 22 seconds because the closed Radix popover node remained in the DOM with `data-state="closed"` even though no trigger had `aria-expanded="true"`; (3) FAIL — after the queue and retained-node fixes, one 300px audit traversed and restored `Gallery/Host Environments/DefaultLeaf`, but its stored promise remained pending after more than 18 seconds with a clean error buffer, while an independent resolved-Promise probe fulfilled normally. Live recheck is pending after the background-layout fix; per task boundary, this follow-up did not use the vault or GUI. |
|    9 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|   10 | PENDING  | —                                             | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
