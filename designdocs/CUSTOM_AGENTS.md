...
the top: a short heading, the built-in **Copilot** selected by default, and
every custom agent listed with its icon, name, and description. A divider
follows, and below it sit today's model and effort sections unchanged.
...
| Effort        | Optional. Same effort levels the session offers for that backend.     |
