...
For an older project with no `AGENTS.md`, the open action initializes it once
from the legacy `project.md` body. A marker-owned generated mirror is converted
to that raw body on first open; unmarked user files are preserved. No bulk
migration rewrites existing projects.
