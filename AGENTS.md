...
- `npm run test:runtime` — Gherkin runtime contracts against the pinned opencode binary (macOS/Linux, no credentials); `npm run test:runtime:steps` prints their step vocabulary.
...
- **Cover user-facing Agent Mode behavior with executable Gherkin.** A new or changed user-facing behavior, or a fix for a meaningful regression, that the runtime suite reaches (Agent Mode on the opencode backend) adds or updates a scenario in `runtime-tests/features/`; an internal refactor keeps every scenario passing unchanged. Reuse steps from `npm run test:runtime:steps`, and justify each new step definition in the PR. Behavior the suite cannot reach gets unit and real-Obsidian coverage, and the PR says which behavior that is. → [`Runtime contracts`](./designdocs/agents/TESTING_GUIDE.md#runtime-contracts-gherkin)
...
- **Write unit tests as executable specifications.** A developer or agent reading the `describe`/`it` outline should understand the module's purpose and observable contract without opening its implementation. Establish normal behavior before boundary, failure, and regression cases; name each case by its condition and expected outcome. Use concrete fixtures and assert the promised result, not just internal calls. For behavior changes, verify the test fails for the intended reason before making it pass, then refactor with tests green. → [`Test design workflow`](./designdocs/agents/TESTING_GUIDE.md#test-design-workflow)
