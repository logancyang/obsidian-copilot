...
After a successful managed installation, and when Copilot starts, completed older managed versions are removed to free disk space. The selected version, equal or newer versions, custom installations, and unfinished downloads are kept. Cleanup failures do not block Agent Chat and are retried on a later startup or installation. If another vault uses a reclaimed version, restart its agent or reinstall through **Configure**. Concurrent use of older managed versions across vaults is not protected during cleanup.
