...
- **Unified view**: Additions and removals shown inline in a single column
- **Side-by-side view**: The original and proposed versions shown in two columns

You can switch views with the toggle at the top of the preview, or set a default in **Settings → Copilot → Plus → Diff View Mode**.

Review the proposed change and click:

- **Apply** — Write the change to your note
- **Cancel** — Discard without making any changes

For finer control, expand the **Decisions** panel below the diff to accept or reject individual added and removed lines, then click **Apply** to write only the lines you kept. **Accept all** and **Reject all** set every line at once.
