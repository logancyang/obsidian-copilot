...
With a Markdown note active, run **Publish file to OpenArtifacts** from the command palette to hand publishing to Agent Chat. Copilot opens Agent Chat and sends the request in a new chat with your configured default agent. Chats already open are never interrupted or changed. Because publishing runs inside Agent Chat, the command is desktop only.
