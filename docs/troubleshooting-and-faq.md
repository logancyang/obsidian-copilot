...
2. Choose **Download & install** under **Managed by Copilot**. If it fails, the same error and **Retry** action appear in Settings and Agent Chat.
...
4. Run the exact-version `npx … cli login` command shown in the dialog if Codex is not authenticated.
