...
Copilot names its Keychain entries by purpose so they are easy to recognize and manage. For example, Gemini, Supadata, and Copilot Plus credentials appear as `copilot-gemini-api-key-v…`, `copilot-supadata-api-key-v…`, and `copilot-plus-license-v…`. Provider-specific keys put the provider label first, such as `copilot-anthropic-team-api-key-p…-v…`; the short provider and vault suffixes distinguish multiple accounts and vaults. Existing Copilot credentials gain readable copies automatically, while their legacy entries remain synchronized as compatibility backups. You do not need to re-enter their values, and **Delete All Keys** removes both forms.
