...
**Errors:** 400 (invalid), 409 (already registered, overlapping folders, or duplicate folder name). Copilot accepts a conflict only after confirming the requested vault path is already registered; otherwise, the connection stays incomplete and registration-failure guidance remains available. If verification does not respond within eight seconds, Copilot reports the original conflict.
