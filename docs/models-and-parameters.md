...
BYOK supports chat models only. Recognized embedding models are excluded from model choices, including manual entries. Upgrading removes previously saved BYOK embedding models and their selections while preserving provider connections, keys, and chat models. Use Miyo for semantic search.
