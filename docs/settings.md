...
> **Self-Host Mode is not a firewall.** It flags and sorts cloud options but does not disable them. Claude, Codex, Copilot-hosted models, cloud BYOK providers, Firecrawl, Perplexity, and Supadata can still send data off-device when selected.
...
| **Web Search Provider**   | **Firecrawl**  | Enabled only while Self-Host Mode is on. Chooses **Firecrawl** or **Perplexity Sonar** as the web-search skill provider.                                                                                                                                          |
...
One place to report a problem and to control the two logs a report can carry.

| Control                     | Default        | What it does                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Report an issue**         | Not applicable | Desktop-only. Asks what went wrong and which attachments to include, packs them into one zip you can review before sending, uploads it privately, and opens a prefilled GitHub issue carrying only the report ID. Nothing leaves the device until you press **Upload & open issue**, and nothing on the issue can download the bundle. For opencode, its global backend log is an extra checkbox that is off by default. |
| **Debug Mode**              | Off            | Logs Quick Chat activity to **View → Toggle Developer Tools**, and pre-selects the chat log when you report an issue. It does not control the Agent Chat activity log below.                                                                                                                                                                                                                                             |
| **Agent Mode activity log** | On             | Records messages between Copilot and the agent so a report always has recent activity to attach. The plain-text log is device-local and outside the vault, but it can contain prompts, note contents, and tool inputs or outputs. Turn it off to stop future logging. **Open** shows the current log on disk; **Clear** deletes it.                                                                                      |

The regular chat log can still be saved by hand: run **Copilot: Create log file** from the command palette to write it into the vault as a note and open it.
