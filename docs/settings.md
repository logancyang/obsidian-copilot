...
> **Simplified Chinese:** When Obsidian's interface language is Simplified Chinese, Copilot follows it across every Settings tab and the Agent Chat critical path, including settings-owned provider, Miyo, command, skill, and confirmation dialogs. Product and feature names such as BYOK, Miyo, Relay, and Plus stay unchanged, as do technical terms such as API, provider and model names, plan names, commands, prompt content, your content, AI output or reasoning, tool calls, logs, paths, and raw backend output. Unsupported or untranslated messages fall back to English.
