...
Codex uses the `codex-acp` adapter and the login held by the Codex CLI. It does not use an OpenAI API key from the BYOK tab. Managed downloads use the adapter version pinned by Copilot. For an existing or manually installed adapter, **Configure** reports any required compatibility update. See [Codex installation details](agent-mode-and-tools.md#codex).
