...
| Control                              | Default                                   | What it does                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------ | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Default model**                    | **Agent default** until explicitly chosen | Sets the model for new chats and multi-agent answerers on that agent. An already open chat adopts a changed default on its next turn. A model disabled later remains visible as disabled until you clear or replace the default. New chats start on one of your enabled models instead of that one, and Copilot tells you once which model is no longer offered. |
...
| **Upgrade to latest / Run opencode upgrade** | Shown when an update is needed        | **Upgrade to latest** downloads Copilot's pinned managed build, not the latest upstream release. **Run opencode upgrade** explicitly runs a custom binary's own upgrade command. You can instead update the custom installation yourself and reopen this dialog, which re-reads the installed version. |
...
For an outdated adapter, select **Configure** from the warning, then choose **Upgrade** for a managed installation or **Download & install** to switch to one. To keep a custom installation, update it yourself and reopen **Configure**, which re-reads the adapter's version. Copilot does not update custom Codex binaries.
...
| -------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Semantic search for agents**   | Off                                        | Desktop-only, and hidden on mobile, where Agent Chat does not run. Requires a reachable Miyo to turn on. Enabling installs the shared `miyo-search` skill; disabling removes it. The switch remains available if Miyo later goes offline. Searches are local and unlimited when Miyo is local.                                                                                                                                                                                                                                                                                                                                        |
...
Each built-in skill has per-agent icon switches. While a change is saving, only that skill’s controls are temporarily unavailable. Choose **Disable skill** from its overflow menu to disable it for every agent. Disabling the whole skill removes its entire folder and agent links, including any files you added inside the folder, while leaving its row visible so you can re-enable it. Keep custom skills and files in separate folders. Turning off one agent removes that agent's access and remembers your choice. Restarting Obsidian, updating Copilot, or changing the Copilot folder does not reset these choices. Re-enabling a skill preserves its per-agent opt-outs. Only changes to the defaults are saved. Restoring all defaults clears that skill’s preference entry; untouched and newly added skills need no entry. Retired skills lose their saved preferences and entire folders, and renamed skills start with fresh defaults.

Copilot generates a built-in skill only when at least one available agent can use it. Agents that have not been set up do not receive skill links or new skill directories. Existing shared skill files are retained when an agent is temporarily unavailable on this device. When another agent becomes available, Copilot automatically enables eligible built-in skills for it unless you previously disabled that skill or that agent's access to it. Miyo Search and Miyo Parse appear only when their corresponding Miyo settings are enabled. Disabling every current agent individually does not disable the skill for future agents; choose **Disable skill** for that.
...
One place to report a problem, plus the switches for the two logs a report can carry.

| Control                     | Default        | What it does                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Report an issue**         | Not applicable | Desktop-only, in two pages. First, describe what went wrong and tick which attachments to include; then review what was actually packed into a single zip and upload it. Copilot uploads that zip privately and opens a prefilled GitHub issue carrying only the report ID. Nothing leaves this device until you choose to upload, and nothing on the issue itself can download the bundle. |
| **Debug Mode**              | Off            | Logs Quick Chat activity to **View → Toggle Developer Tools**. Turn it on and reproduce the problem before you report an issue, so the chat log a report collects actually contains it. It does not control the Agent Mode activity log below.                                                                                                                                              |
| **Agent Mode activity log** | On             | Records the messages between Copilot and the agent, so a report always has recent activity to attach. The log is plain text, lives on this device outside your vault, and can contain your prompts, note contents, and tool inputs or outputs. Turn it off to stop further logging. On the same row, **Open** shows the log file on disk and **Clear** deletes it; both are desktop-only.   |

The regular chat log can still be saved by hand whenever you want it: run **Copilot: Create log file** from the command palette to write it into your vault as a note and open it.
