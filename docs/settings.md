...
| **Sign in**                                         | Not applicable                           | Shows `codex-acp cli login`. Run it outside Copilot; the adapter uses the bundled Codex CLI to store that login. There is no key field here.                      |
...
When Copilot first registers a vault, it sends its current and historical working folders and Obsidian's ignored paths as the folder's initial Miyo exclusions. Copilot does not overwrite an existing registration or later Miyo edits. It applies its current working-folder history and user-authored inclusion and exclusion rules to integrated Miyo results at query time. Manage the registered folder's server-side scope in Miyo; those rules also determine what any enabled Relay client can read.
