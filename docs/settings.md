...
| **Document Processor**           | **Plus**                                   | **Plus** sends supported documents to Copilot's hosted processor and can use paid credits. **Miyo** processes PDF and EPUB with the local Miyo CLI installed on this computer; other formats still use Plus. It does not send parsing to a configured remote Miyo server. If the local CLI is unavailable, processing stops with an error instead of silently falling back to the cloud. This selector stays available while disconnected so you can switch back to Plus. |
