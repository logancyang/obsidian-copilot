...
Each built-in skill has per-agent icon switches. While a change is saving, only that skill’s controls are temporarily unavailable. Choose **Disable skill** from its overflow menu to disable it for every agent. Disabling the whole skill removes its generated files and agent links, while leaving its row visible so you can re-enable it. Files you added alongside the bundled files are preserved. Turning off one agent removes that agent's access and remembers your choice. Restarting Obsidian, updating Copilot, or changing the Copilot folder does not reset these choices. Re-enabling a skill preserves its per-agent opt-outs. Only changes to the defaults are saved. Restoring all defaults clears that skill’s preference entry; untouched and newly added skills need no entry. Retired skills lose their saved preferences and bundled files, and renamed skills start with fresh defaults.
