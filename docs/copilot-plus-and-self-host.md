...
- **AI boost** in Open Copilot search sends the query, local time, vault name, and up to 150 candidate files' names, folders, types, dates, sizes, tags, Miyo scores and ranks to Brevilabs. Miyo-sourced candidates also include up to 600 characters from their best matching plain-text excerpt. When it fits the service's request budget, AI boost also sends an inventory of every file in the checked file types with its vault-relative path, type, size, and created and modified dates. For larger vaults, it sends file counts by type instead of that inventory; the candidate files still include their rank by size and dates. AI boost is on by default for licensed users and can be turned off in the search modal.
