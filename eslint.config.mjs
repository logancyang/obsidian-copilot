...
export default [
  {
    ignores: [
      ...
      "docs/**",
      ".claude/**",
      // Generated by the runtime suite: its bundle, cached binary, and run reports.
      "runtime-tests/.build/**",
  ...
  // The runtime suite is Node test code that drives a real subprocess; it is
  // never bundled into the plugin, so the rules guarding Obsidian's mobile and
  // popout runtimes do not apply to it.
  {
    files: ["runtime-tests/**/*.{ts,mjs}"],
    rules: {
      "obsidianmd/no-nodejs-modules": "off",
